...
export async function GET (req:NextRequest){
    ...
    if(url==null ||typeof url !="string" || url.length==0)
        return NextResponse.json({status: 'Ok',method:"Number of pages" , error:"missing or invalid url !"},{status: 400});
    ...
    return NextResponse.json({status: 'Ok',method:"Number of pages" ,number_of_pages:np },{status: 200});
} 
